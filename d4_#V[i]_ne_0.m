  ...
  for i in [1..#P] do if #V[i] le 1 then continue i; end if; v:=#V[i];
   ...
   if b then vprintf QuotientFD,2: "Prime %o: pt %o image: %o\n",pp,P[i],im;
    ...
    if v ne #V[i] then vprintf QuotientFD,1: "%o reduces Im(%o) to size %o\n",pp,P[i],#V[i];
     if #V[i] eq 1 then vprintf QuotientFD,2: "Image is %o\n",Random(V[i]); end if;
    end if;
   end if;
   if #V[i] eq 0 then
    vprintf QuotientFD,2: "\nreduced V[i] size to zero?? (not good, re-run or expect higher than optimal number of 4-coverings)\n\n";
