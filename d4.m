...
function d4_remove(f,root,gens,RB,RG,QA)
 ...
 V:=[U : i in [1..#P]]; START:=2; pp:=NextPrime(START); WARN:=START+2000;
  ...
  if pp gt WARN then WARN:=WARN+2000;
   ...
   for i in [1 .. #P] do if #V[i] le 1 then continue i; end if;
    ...
    for k in V[i] do
     ...
     C[k] := Minimize(GenusOneModel(EpsilonToCover(QA!&*A)));
    end for;

// changes to block 1 were done on July 17, 2026
// --- start block 1 ---
     x:=P[i][1]; y:=P[i][2]; //I:=cInvariants(Curve(P[i]))[1];
     //print "#cInvariants(Curve(P[i])) = ", #cInvariants(Curve(P[i]));
     //print "cInvariants(Curve(P[i])) = ", cInvariants(Curve(P[i]));
     // G:=Minimize(GenusOneModel([1,0,-x/6,y/27,I/12-x^2/432])); // maybe don't use this one
     // G:=Minimize(GenusOneModel([1,0,-x/6,y/27,(I/36-x^2)/432]));
     G:=GenusOneModel(2, P[i]); // would also work here
     // k0 := Random(V[i]);
     if exists(im){k : k in V[i] | IsEquivalent(AddCovers(G, C[k0]), C[k])} then // instead of assert
      V[i]:={im-k0};
     else
      DP := DivisionPoints(P[i], 2);
      if #DP eq 0 then
       P[i] := Random([-1, 1])*2*P[i];
      else
       P[i] := Random(DP);
      end if;
     end if;
   end for;
   // break; // Why was this break here? Commented on July 17, 2026
  end if;
// --- end block 1 ---
  ...
  for i in [1..#P] do if #V[i] le 1 then continue i; end if; v:=#V[i];
   ...
   if b then vprintf QuotientFD,2: "Prime %o: pt %o image: %o\n",pp,P[i],im;
    ...
    if not b then vprintf QuotientFD,1: "Hmm no idea about prime %o\n",pp; continue; end if; // this fails in some cases ? // assert b;
    Vi_old := V[i]; // trying to avoid #V[i] eq 0
    V[i]:=V[i] meet Set([s+k : k in K]);
    if v ne #V[i] then vprintf QuotientFD,1: "%o reduces Im(%o) to size %o\n",pp,P[i],#V[i];
     if #V[i] eq 1 then
      vprintf QuotientFD, 2: "Image is %o\n", Random(V[i]);
      vprint QuotientFD, 1: "#V[i] = 1 (good, ignore previous #V[i]=0 warings, if any)";
     end if;
    end if;
   end if;
   if #V[i] eq 0 then
    vprint QuotientFD, 1: "reduced V[i] size to zero?? (not good, re-run or expect higher than optimal number of 4-coverings)";
    // vprintf QuotientFD,2: "Vi_old = %o\n", Vi_old;
    V[i] := Vi_old; // trying to avoid #V[i] eq 0
   end if;
  end for;
  if &and[#v le 1: v in V] then break; end if;
 end while;
 W:=sub<R|[Random(v) : v in V | #v eq 1]>; //hope this termination suffices..
 return true,W*Matrix(Basis(gens));
end function;
...
{Constructs the set of everywhere-locally-solvable covers for
 ...
 vprintf FourDescent,1: "Making explicit four-coverings\n";
...
/*
  // Remove coverings generated by RemoveGensEC.
  if #RemoveGensEC gt 0 then
      gens := Setseq(RemoveGensEC);
      r := #gens;
      // Build models for the known coverings.
      remove_models := [];
      // Work modulo sign automatically by storing only one of Q,-Q.
      seen := {};
      for cc in CartesianPower({0, 1, 2, 3}, r) do
          Q := E!0;
          for i in [1 .. r] do
              Q +:= cc[i]*gens[i];
          end for;
          if Q ne E!0 then
              // Use the point itself as the key; the opposite sign will be skipped later.
              if Q notin seen and -Q notin seen then
                  Include(~seen, Q);
                  C := GenusOneModel(4, Q);
                  C := Minimise(C);
                  C := Reduce(C);
                  Append(~remove_models, C);
              end if;
          end if;
      end for;
      // print "#remove_models = ", #remove_models;
      // print "#seen = ", #seen;

      // Compare only genus-one models.
      keep := [];
      for i in [1 .. #models] do
          C := models[i];
          is_removed := false;
          for j in [1 .. #remove_models] do
              // print "i = ", i, ", j =  ", j;
              D := remove_models[j];
              if IsEquivalent(C, D) then
                  is_removed := true;
                  print "C = ", C, ", D = ", D;
                  break;
              end if;
